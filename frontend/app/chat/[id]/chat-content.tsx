  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const negotiationResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/negotiate`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (negotiationResponse.ok) {
        ...
        setAnalysisResults(
          negotiationData.options.map((text: string, i: number) => ({
            ...
            score:
              i === 0
                ? negotiationData.state_evaluation
                : negotiationData.state_evaluation * (0.9 - i * 0.1),
          }))
        );
      }
    } catch (error) {
        ...
        >
          <div className="flex-1 border-r">
            <div className="flex flex-col h-screen">
              <div className="sticky top-0 border-b shadow-sm bg-background p-4 flex items-center gap-4">
                <Button
                  onClick={() => {
                    if (isMobile) {
            ...
            <div className="sm:w-[400px] size-full space-y-4 p-4 py-0">
              ...
              <div className="space-y-2">
                <h3 className="text-sm font-medium">
                  CONVERSATIONAL STATE EXPLORATION
                </h3>
                <DotGrid
                  color="primary"
                  rows={12}
                  ...
                  } promising states explored out of ${
                    nodes.length
                  } total states`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <DotGrid
                  ...
                  activeNodes={
                    nodes.filter(
                      (n) =>
                        n.status === "evaluating" || n.evaluation_score !== null
                    ).length
                  }
                  maxNodes={nodes.length}
                  tooltip={`${
                  ...
                  } states evaluated with max ${Math.max(
                    ...nodes.map((n) => n.visits),
                    0
                  )} visits`}
                />
              </div>
...
function DotGrid({
  ...
  const normalizedActive = maxNodes
    ? Math.floor((activeNodes / maxNodes) * totalDots)
    : activeNodes;
